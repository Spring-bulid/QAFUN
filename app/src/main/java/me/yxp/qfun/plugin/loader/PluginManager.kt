...
object PluginManager {
    ...
    private const val DEFAULT_DESC = "这是一个自动生成的 TypeScript 示例脚本。"
    private val DEFAULT_CODE = """
        // QAFUN TypeScript 插件示例
        // 文档: 同目录 main.ts 为入口，顶层代码启动即执行
        // 全局 API: sendMsg / sendPic / toast / qqToast / addItem / addMenuItem / onMsg / ...
        ...
        // 悬浮球菜单回调 (3 参或 4 参)
        function onTestClick(chatType: number, peerUin: string, peerName: string) {
            qqToast(2, "点击了菜单: " + peerName);
            sendMsg(peerUin, "来自 QAFUN TS 插件", chatType);
        }

        // 收到消息回调
        function onMsg(msg: any) {
            if (msg.msg.indexOf("测试") >= 0) {
                qqToast(2, "收到测试消息: " + msg.msg);
            }
        }
    ...
    fun createPlugin(
        ...
        try {
            ...
            val mainFile = File(targetDir, "main.ts")
